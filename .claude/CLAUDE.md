...
- `maps.princexizor.ddns.net` — Leaflet interactive atlas over `PatrinorModern.png`
  ...
  session logs). Static bundle lives in `maps-site/`. The place-name index
  `maps-site/data/gazetteer.json` (name+kind+footprint+wiki+etym) is generated by
  `tools/build-gazetteer.js` — regenerate after editing marker/anchor data or
  etymologies; don't hand-edit. `maps-site/lexicon.html` is the Peternek Hag
  dictionary (accepts `?q=` deep links); its data comes from `lexicon/patrinaic.json`.
  Verified research digests that drive atlas/sim roadmap items live in
  `docs/research/`.
